...
def convert_vmaf_scores(input_file, output_file):
    ...
    for row in data:
        ...
        match = re.match(r'.*?_(\d).*?_c(\d).*?_qp(\d+)\.mp4', case)
        if match:
            rq, cam, qp = match.groups()

            # Construct the new row for the output CSV
            output_data.append([cam, rq, qp, vmaf_score])

    # Write the output CSV file
    with open(output_file, 'w', newline='') as outfile:
        writer = csv.writer(outfile)
        writer.writerow(['cam', 'rq', 'qp', 'vmaf'])  # Write header
        writer.writerows(output_data)

# Example usage
input_file = './vmaf_results.csv'
output_file = 'output_cam.csv'
